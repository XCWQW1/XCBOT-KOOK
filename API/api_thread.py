    ...
    if sys.platform == 'win32':
        ...
        def thread_func():
            try:
                ...
                Log.error("error", f"线程报错：{inspect.getsourcefile(func)} | {traceback.format_exc()}")
                result_queue.put(e_1)
        ...
        try:
            ...
            Log.error("error", f"多线程报错：{inspect.getsourcefile(func)} | {traceback.format_exc()}")
            return None
        ...
        for result in results:
            ...
            if isinstance(result, Exception):
                ...
                with open(log_file, 'a') as f_log:
                    ...
                    trace = ''.join(traceback.format_exception(type(result), result, result.__traceback__))
                    # 设置日志内容
                    logs = f"[{Log.now_time()}] [错误] [线] {inspect.getsourcefile(func)} | {traceback.format_exc()}"
                    # 显示日志
                    print(logs)
                    f_log.write(f"子线程执行出错: {inspect.getsourcefile(func)} | {traceback.format_exc()}\n")
                return None
